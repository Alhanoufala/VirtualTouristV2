...
class FlickrClient {
    ...
    class func getSearchForPhotos(latitude:Double ,longitude:Double,completion : @escaping ([PhotoParserResponse],Error?)-> Void ) {
        taskForGETRequest(url: EndPoint.searchForPhotos(latitude: latitude, longitude: longitude).url, responseType: SearchResponse.self) { response , error in
